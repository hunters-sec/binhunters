...
public class FunctionGraphFactory {
	...
	private static Collection<FGEdge> createdEdges(BidiMap<CodeBlock, FGVertex> vertices,
		...
		assignConditionalBranchLabels(edges);

		return edges;
	}

	/**
	 * Assigns "T" (taken) and "F" (fallthrough) labels to edges leaving vertices
	 * with conditional branches. This helps users understand which path each edge
	 * represents in the function graph.
	 *
	 * <p>For each source vertex, if there are exactly two outgoing edges where one is
	 * conditional and the other is fallthrough, labels them "T" and "F" respectively.
	 */
	private static void assignConditionalBranchLabels(List<FGEdge> edges) {
		// Group edges by their source vertex
		Map<FGVertex, List<FGEdge>> edgesBySource = new HashMap<>();
		for (FGEdge edge : edges) {
			edgesBySource.computeIfAbsent(edge.getStart(), k -> new ArrayList<>()).add(edge);
		}

		for (List<FGEdge> outEdges : edgesBySource.values()) {
			if (outEdges.size() != 2) {
				continue; // only label simple two-way conditional branches
			}

			FGEdge first = outEdges.get(0);
			FGEdge second = outEdges.get(1);
			FlowType ft1 = first.getFlowType();
			FlowType ft2 = second.getFlowType();

			if (ft1 == null || ft2 == null) {
				continue;
			}

			// Look for conditional + fallthrough pair
			if (ft1.isConditional() && ft2.hasFallthrough() && !ft2.isConditional()) {
				first.setLabel("T");
				second.setLabel("F");
			}
			else if (ft2.isConditional() && ft1.hasFallthrough() && !ft1.isConditional()) {
				second.setLabel("T");
				first.setLabel("F");
			}
		}
