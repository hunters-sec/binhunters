...

{
  ...
  // Also look for variables that hold function return values and name them
  // based on the called function. E.g., a variable holding the return of
  // malloc() gets named "mallocResult" instead of "pvVar1".
  for(int4 i=0;i<numfunc;++i) {
    FuncCallSpecs *fc = data.getCallSpecs(i);
    PcodeOp *op = fc->getOp();
    Varnode *outvn = op->getOut();
    if (outvn == (Varnode *)0) continue;	// No return value
    HighVariable *high = outvn->getHigh();
    if (high == (HighVariable *)0) continue;
    if (high->isAddrTied()) continue;
    if (high->getNumMergeClasses() > 1) continue;
    Symbol *sym = high->getSymbol();
    if (sym == (Symbol *)0) continue;
    if (!sym->isNameUndefined()) continue;
    // Already has a name recommendation from parameter propagation, skip
    if (recmap.find(high) != recmap.end()) continue;

    string funcname = fc->getName();
    if (funcname.empty()) continue;
    // Strip leading underscores from function names like _malloc, __strlen
    while (!funcname.empty() && funcname[0] == '_')
      funcname = funcname.substr(1);
    if (funcname.empty()) continue;
    if (funcname.size() > 20) continue;	// Don't use very long function names

    OpRecommend oprec;
    oprec.ct = (Datatype *)0;
    oprec.namerec = funcname + "Result";
    recmap[high] = oprec;
  }
...

{
  ...
  act->addAction( new ActionFuncLinkOutOnly("noproto") );
  {
	...
	actprop->addRule( new RuleSextEliminate("analysis") );
	actprop->addRule( new RuleZextCompareConst("analysis") );
