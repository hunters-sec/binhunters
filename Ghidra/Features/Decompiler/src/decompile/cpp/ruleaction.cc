...
/// \class RuleSextEliminate
/// \brief Eliminate INT_SEXT in signed comparisons:  `sext(V) s< c  =>  V s< c`
///
/// Mirror of RuleZextEliminate for signed extensions.
/// The constant must fit within the smaller signed range.
/// Handle INT_SLESS, INT_SLESSEQUAL, INT_EQUAL, and INT_NOTEQUAL.
void RuleSextEliminate::getOpList(vector<uint4> &oplist) const
...
int4 RuleSextEliminate::applyOp(PcodeOp *op,Funcdata &data)

{
  PcodeOp *sext;
  Varnode *vn1,*vn2,*newvn;
  intb val;
  int4 smallsize,sextslot,otherslot;

  vn1 = op->getIn(0);
  vn2 = op->getIn(1);
  sextslot = 0;
  otherslot = 1;
  if ((vn2->isWritten())&&(vn2->getDef()->code()==CPUI_INT_SEXT)) {
    vn1 = vn2;
    vn2 = op->getIn(0);
    sextslot = 1;
    otherslot = 0;
  }
  else if ((!vn1->isWritten())||(vn1->getDef()->code()!=CPUI_INT_SEXT))
    return 0;

  if (!vn2->isConstant()) return 0;
  sext = vn1->getDef();
  if (!sext->getIn(0)->isHeritageKnown()) return 0;
  if (vn1->loneDescend() != op) return 0;
  smallsize = sext->getIn(0)->getSize();

  // Check if the constant fits in the signed range of the smaller type
  val = sign_extend(vn2->getOffset(),8*vn2->getSize()-1);
  intb maxval = ((intb)1 << (8*smallsize-1)) - 1;
  intb minval = -((intb)1 << (8*smallsize-1));
  if (val < minval || val > maxval)
    return 0;	// Constant doesn't fit in smaller signed type

  uintb smallval = (uintb)val & calc_mask(smallsize);
  newvn = data.newConstant(smallsize,smallval);
  newvn->copySymbolIfValid(vn2);
  data.opSetInput(op,sext->getIn(0),sextslot);
  data.opSetInput(op,newvn,otherslot);
  return 1;
}

/// \class RuleZextCompareConst
/// \brief Extended zero-extension elimination for comparisons with constants
///
/// Extends RuleZextEliminate to handle more comparison patterns.
/// When `zext(x) <op> c` and the constant `c` fits in the smaller type,
/// and the comparison operator is any of the standard set, simplify.
/// This handles signed comparisons against zero-extended values.
void RuleZextCompareConst::getOpList(vector<uint4> &oplist) const

{
  uint4 list[] = {CPUI_INT_SLESS, CPUI_INT_SLESSEQUAL };
  oplist.insert(oplist.end(),list,list+2);
}

int4 RuleZextCompareConst::applyOp(PcodeOp *op,Funcdata &data)

{
  ...
  // For signed comparisons against a zero-extended value:
  // The zero-extended value is always non-negative, so if the constant
  // is negative, we can resolve statically. If the constant fits in
  // the smaller unsigned range, we can convert to unsigned comparison.
  intb sval = sign_extend(val, 8*vn2->getSize()-1);
  if (sval < 0) {
    // zext(x) is always >= 0, so zext(x) s< negative is always false
    // and zext(x) s>= negative is always true
    // Let other rules handle constant folding
    return 0;
  }
  // Positive constant: check if it fits in the smaller type
  if ((val >> (8*smallsize)) == 0) {
    // Convert signed comparison to unsigned since both sides are non-negative
    newvn = data.newConstant(smallsize,val);
    newvn->copySymbolIfValid(vn2);
    data.opSetInput(op,zext->getIn(0),zextslot);
    data.opSetInput(op,newvn,otherslot);
    // Convert SLESS -> LESS, SLESSEQUAL -> LESSEQUAL since values are non-negative
    if (op->code() == CPUI_INT_SLESS)
      data.opSetOpcode(op,CPUI_INT_LESS);
    else
      data.opSetOpcode(op,CPUI_INT_LESSEQUAL);
