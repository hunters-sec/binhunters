...

{
  ...
  if (outVn->isExplicit()) {
	...
	if (otherVn->isConstant()) {
	  ...
	  // If the constant fits within the smaller input type, the extension is implied
	  // by the comparison semantics, no explicit cast needed
	  break;
	}
	...
	break;
      ...
      case CPUI_INT_RIGHT:
      case CPUI_INT_SRIGHT:
	// Shift operations: the extension of the value being shifted is implied
	// when it's the value operand (slot 0), not the shift amount
	slot = readOp->getSlot(outVn);
	if (slot != 0)
	  return false;	// Extension of shift amount is not naturally implied
	break;
      case CPUI_STORE:
      case CPUI_RETURN:
	// Extension before a store or return is implied by the assignment context
	break;
      case CPUI_CALL:
      case CPUI_CALLIND:
	// Extension when passing to a function call is implied by parameter promotion
	break;
      default:
	return false;
    }
...
{				// Generic casting rules that apply for most ops
  ...
  case TYPE_UINT:
	...
	return (Datatype *)0;
      // Suppress int-to-uint cast for same-size integer types in non-pointer contexts.
      // The bit pattern is identical so the cast is pure visual noise in arithmetic,
      // bitwise, and assignment contexts. This eliminates most ugly (uint) casts.
      if ((meta == TYPE_INT) && (reqbase->getSize() == curbase->getSize()) && !isptr)
	return (Datatype *)0;
  ...
  case TYPE_INT:
	...
	return (Datatype *)0;	// Can be TYPE_INT for typedef/enumerated/char
      // Suppress uint-to-int cast for same-size integer types in non-pointer contexts.
      // Bit pattern is identical, cast adds visual noise without semantic value.
      if ((meta == TYPE_UINT) && (reqbase->getSize() == curbase->getSize()) && !isptr)
	return (Datatype *)0;
      if (isptr && (meta==TYPE_UNKNOWN || meta==TYPE_PARTIALSTRUCT || meta==TYPE_PARTIALUNION))
	return (Datatype *)0;	 // Don't cast pointers to unknown
    }
