					...
					Datatype *ct,int4 &index,uint4 flags) const
{
  ...
  if ((flags & Varnode::unaffected)!=0) {
    ...
    // Generate more descriptive parameter names based on type when available
    if (ct != (Datatype *)0) {
      type_metatype meta = ct->getMetatype();
      if (meta == TYPE_PTR) {
	Datatype *ptrTo = ((TypePointer *)ct)->getPtrTo();
	if (ptrTo != (Datatype *)0 && ptrTo->getMetatype() == TYPE_INT && ptrTo->getSize() == 1)
	  s << "param_str" << dec << index;	// char* -> param_str
	else if (ptrTo != (Datatype *)0 && ptrTo->getMetatype() == TYPE_VOID)
	  s << "param_ptr" << dec << index;	// void* -> param_ptr
	else {
	  s << "param_" << dec << index;
	}
      }
