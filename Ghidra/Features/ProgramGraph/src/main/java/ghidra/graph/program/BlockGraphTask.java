...
public class BlockGraphTask extends Task {
	...
	private int codeLimitPerBlock = 20;
	...
	protected void setEdgeAttributes(AttributedEdge edge, CodeBlockReference ref) {
		...
		edge.setEdgeType(ProgramGraphType.getEdgeType(flowType));

		// Add a human-readable flow label for better graph readability
		String flowLabel = getFlowLabel(flowType);
		if (flowLabel != null) {
			edge.setAttribute("EdgeLabel", flowLabel);
		}
	}

	private String getFlowLabel(FlowType flowType) {
		if (flowType.isConditional()) {
			return "Cond";
		}
		if (flowType.hasFallthrough() && !flowType.isCall() && !flowType.isConditional()) {
			return "Fall";
		}
		if (flowType.isCall()) {
			return "Call";
		}
		if (flowType.isUnConditional() && !flowType.hasFallthrough()) {
			return "Jump";
		}
		return null;
	}
	...
	protected void setVertexAttributes(AttributedVertex vertex, CodeBlock bb, boolean isEntry) {
		...
		// Add address range for context
		vertex.setAttribute("Address",
			firstStartAddress.toString() + " - " + bb.getMaxAddress().toString());
	}
