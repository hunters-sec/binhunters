...
//==================================================================================================
	...
	private FcgEdge getOrCreateEdge(FcgVertex start, FcgVertex end) {
		...
		FcgEdge newEdge = new FcgEdge(start, end);

		// Compute call count: how many references from start function to end function
		int callCount = computeCallCount(start.getFunction(), end.getFunction());
		if (callCount > 1) {
			newEdge.setCallCount(callCount);
		}
		return newEdge;
	}

	private int computeCallCount(Function caller, Function callee) {
		if (caller == null || callee == null) {
			return 1;
		}
		Program program = caller.getProgram();
		if (program == null) {
			return 1;
		}
		Address calleeEntry = callee.getEntryPoint();
		ghidra.program.model.symbol.ReferenceManager refMgr = program.getReferenceManager();
		ghidra.program.model.symbol.ReferenceIterator refs = refMgr.getReferencesTo(calleeEntry);
		int count = 0;
		while (refs.hasNext()) {
