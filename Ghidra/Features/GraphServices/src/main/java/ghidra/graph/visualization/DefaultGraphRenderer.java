...
public class DefaultGraphRenderer implements GraphRenderer {
	...
	private static final int LABEL_WRAP_LENGTH = 120;
	...
	private String getVertexRenderedLabel(AttributedVertex v) {
		...
		return HTMLUtilities.toLiteralHTML(displayName, LABEL_WRAP_LENGTH);
	}
	...
	public void initializeViewer(VisualizationViewer<AttributedVertex, AttributedEdge> viewer) {
		...
		// Enable edge labels (e.g., "Cond", "Fall", "Jump", "Call" for block flow graphs)
		renderContext.setEdgeLabelFunction(this::getEdgeLabel);
		renderContext.setEdgeLabelCloseness(0.5f);
	}
	...
	private void prepareLabel(String vertexName, Color vertexColor) {
		...
		label.setFont(font);
		String escapedText = HTMLUtilities.toLiteralHTML(vertexName, LABEL_WRAP_LENGTH);
	...
	private String getEdgeLabel(AttributedEdge edge) {
		return options.getEdgeLabel(edge);
	}
