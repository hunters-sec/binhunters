...
public class GraphDisplayOptions implements OptionsChangeListener {
	...
	private String edgeLabelOverride = null;
	...
	private int maxNodeCount = 2000; // Binhunters: increased for large function analysis
	...
	/**
	 * Sets the attribute key that can be used to override the label text shown for an edge.
	 * When set, edges with this attribute will display the attribute value as a label.
	 * @param attributeKey the attribute key that, if set, will be used to define the edge's label
	 */
	public void setEdgeLabelOverrideAttributeKey(String attributeKey) {
		edgeLabelOverride = attributeKey;
	}

	/**
	 * Returns the label text for the given edge. If an edge label override attribute is set
	 * and the edge has that attribute, its value is returned. Otherwise returns null (no label).
	 * @param edge the edge for which to get label text
	 * @return the label text or null if no label should be shown
	 */
	public String getEdgeLabel(AttributedEdge edge) {
		if (edgeLabelOverride != null) {
			return edge.getAttribute(edgeLabelOverride);
		}
		return null;
	}
